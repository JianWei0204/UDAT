...
class DomainAwareTrainerMixin:
    ...
    def split_domain_batch(self, batch):
        ...
        if 'domain_label' in batch:
            ...
            source_batch = {k: v[source_mask] if isinstance(v, torch.Tensor) else [v[i] for i in range(len(v)) if source_mask[i]] for k, v in batch.items()}
            target_batch = {k: v[target_mask] if isinstance(v, torch.Tensor) else [v[i] for i in range(len(v)) if target_mask[i]] for k, v in batch.items()}
