...
def parse_args():
    ...
    parser.add_argument('--source_data', type=str, required=True, help='Source domain dataset YAML path')
    parser.add_argument('--target_data', type=str, required=True, help='Target domain dataset YAML path')
    parser.add_argument('--epochs', type=int, default=100, help='Number of training epochs')
    parser.add_argument('--batch', type=int, default=16, help='Batch size')
    parser.add_argument('--imgsz', type=int, default=640, help='Image size')
    parser.add_argument('--device', type=str, default='', help='Device to use (e.g., 0 or 0,1,2,3 or cpu)')
...
def create_domain_config(source_data, target_data):
    ...
    This merges source and target domain configurations and creates
    a structure that the trainer can use to handle both domains.
    """
    ...
    # This ensures compatibility with existing trainer
...
class DomainDataLoader:
    """
    Custom data loader that provides both source and target domain batches.
    
    This is a placeholder implementation. In practice, you would need to
    implement a proper data loader that alternates or combines source and
    target domain data.
    """
    ...
    def __next__(self):
        # Get next batch from source domain
        try:
        ...
        # Get next batch from target domain
        try:
...
def main():
    ...
    training_args = {
        'model': args.model,
        'data': domain_config,  # Use combined domain configuration
        'epochs': args.epochs,
        'batch': args.batch,
        'imgsz': args.imgsz,
        'device': args.device,
    ...
    trainer = DomainAdaptationTrainer(overrides=training_args)
